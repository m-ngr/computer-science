...
(* ========================================== shift ============================================== *)
(* real * real * geom_exp -> geom_exp
	shifts the given geom_exp value by dx and dy. raise exception if geom_exp is not a value
*)
...
fun shift (dx, dy, g) = 
  ...
  | LineSegment(x1,y1,x2,y2)  => LineSegment(x1+dx, y1+dy, x2+dx, y2+dy)
	| _ => raise BadProgram("Cannot Shift a non-value Expression")
;
(* ========================================== eval_prog ========================================== *)
...
fun eval_prog (e,env) =
  ...
  | Intersect(e1,e2) => intersect(eval_prog(e1,env), eval_prog(e2, env))
	| Shift(dx,dy,g) => shift(dx,dy, eval_prog(g, env))
;
(* ====================================== preprocess_prog ======================================== *)
(* geom_exp -> geom_exp
	preprocesses the given geom_exp to another valid geom_exp with the following invariants:
		- convert any LineSegment that has the same endpoints (x,y) to a Point(x,y).
		- reorder the endpoints of the LineSegment to have one of the following invariants in order:
			1- (x1 < x2)
			2- (x1 = x2) and (y1 < y2)
*)
...
fun preprocess_prog e =
  ...
  | Intersect(e1,e2) => Intersect(preprocess_prog(e1), preprocess_prog(e2))
	| Shift(dx,dy,g) => Shift(dx,dy, preprocess_prog(g))
;
