...
#pragma region The Functional Interface
	...
	template<typename F>
	HashSet filter(F& predicate) {
		HashSet result(capacity(), max_load_factor());
		for (auto& item : *this) {
	...
	HashSet<T2, H2> map(T2(*transform) (const value_type&)) {
		HashSet<T2, H2> result(capacity(), max_load_factor());
		for (auto& item : *this) {
	...
	template<typename F>
	HashSet& filter_self(F& predicate) {
	...
	HashSet& remove_all(const value_type& value) {
