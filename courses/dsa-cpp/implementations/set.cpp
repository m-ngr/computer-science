#include<vector>
/* AVL-based Set */
template<typename T, typename C = std::vector<T>> class Set {
public:
...
#pragma region The Class Interface
	...
	Set(const Set& tree) { copy_tree(tree); }

	Set(Set&& tree) { move_tree(tree); }
...
#pragma region The Functional Interface
	...
	Set<T2, C2> map(T2(*transform) (const value_type&)) {
		Set<T2, C2> result;
		for (auto& item : *this) {
	...
	Set& remove_all(const value_type& value) {
...
template<typename T, typename C> class Set<T,C>::BinaryNode {
public:
	value_type value{};
	int height{ 0 };
	BinaryNode* parent{};
...
template<typename T, typename C> class Set<T,C>::Iterator {
public:
