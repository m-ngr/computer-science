...
template<typename T, typename C = std::vector<T>> class BinarySearchTree {
public:
...
#pragma region The Class Interface
	...
	~BinarySearchTree() { remove_tree(); }

	BinarySearchTree(const std::initializer_list<value_type>& items) {
		insert(items);
	}
...
#pragma region Helper Functions
	...
	void copy_tree(const BinarySearchTree& tree) {
	...
	void move_tree(BinarySearchTree& tree) {
		set_root(tree.root());
		_size = tree._size;
...
template<typename T, typename C> class BinarySearchTree<T,C>::BinaryNode {
public:
	value_type value;
	BinaryNode* parent{};
	BinaryNode* left{};
...
template<typename T, typename C> class BinarySearchTree<T,C>::Iterator {
public:
