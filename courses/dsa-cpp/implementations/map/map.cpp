...
#pragma region The Class Interface
	...
	Map(const std::initializer_list<Entry>& items) {
		insert(items);
	}

	Map(const Map& tree) { copy_tree(tree); }

	Map(Map&& tree) { move_tree(tree); }

	Map& operator=(const Map& tree) {
		remove_tree();
		copy_tree(tree);
		return *this;
	}
...
#pragma region The Standard Interface
	...
	Iterator find(const key_type& key) {
		...
		while (node) {
			if (node->entry.key() == key) return Iterator(node);
...
#pragma region The Functional Interface
	...
	Map<K2, V2> map(typename Map<K2, V2>::Entry(*transform) (const Entry&)) {
		Map<K2, V2> result;
		for (auto& entry : *this) {
	...
	Map& remove_all(const value_type& value) {
	...
	BinaryNode* inserter(const Entry& item, BinaryNode* node) {
		while (node) {
			if (item.key() == node->entry.key()) {
				node->entry.set_value(item.value());
				return node;
			} else if (item.key() < node->entry.key()) {
				if (!node->left) {
...
template<typename K, typename V> class Map<K,V>::BinaryNode {
public:
	Entry entry{};
	int height{ 0 };
	BinaryNode* parent{};
...
template<typename K, typename V> class Map<K,V>::Iterator {
public:
...
template<typename K, typename V> class Map<K,V>::Entry {
public:
