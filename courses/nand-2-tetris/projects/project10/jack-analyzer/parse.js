...
// @TODO: NEED REFACTOR
function parseTerm_OLD() {
  const value = [];

  try {
    return makeNode(
      [expectType(NODE_TYPE.INT, NODE_TYPE.STRING)],
      NODE_TYPE.TERM,
      false
    );
  } catch (error) {}

  if (KW_CONST.includes(ts.next().value)) {
    return makeNode([ts.get()], NODE_TYPE.TERM, false);
  }
  ...
  if (UNARY_OP.includes(ts.next().value)) {
    value.push(ts.get(), parseTerm());
    return makeNode(value, NODE_TYPE.TERM, false);
  }

  const identifier = expectType(NODE_TYPE.IDENTIFIER);
