 ...
 * 
 * List Format: explicit free-list (doubly-linked list)
 * Heap Format: [Head-Block[1] | Regular-Blocks ...| Tail-Block[1]]
...
// #define DEBUG                 /* uncomment to turn-on heap checker */
  ...
  static void check_free_list(int line, int free_count);
...
static WTYPE* free_list = NULL;      /* pointer to first free-block payload */
...
static void link_block(void* pp){
  if (free_list) SET_PRED(free_list, pp);
  SET_SUCC(pp, free_list);
  SET_PRED(pp, NULL);
  free_list = pp;
}
...
static void unlink_block(void* pp) {
  ...
  if (pp == free_list) free_list = succ_pp;
}
...
static void mm_check(int line){
  ...
  // check Free-List
  check_free_list(line, free_count);
}
...
*/
static void check_free_list(int line, int free_count){
  void* start_ptr = mem_heap_lo();
  void* end_ptr = MOVE_BYTE(start_ptr, mem_heapsize()) - 1;
  void* pred_pp = NULL;
  int count = 0;
  for(void* pp = free_list; pp != NULL; pp = GET_SUCC(pp)){
    ...
    if (pred_pp != GET_PRED(pp)){
      printf("Error at %d: inconsistant predecessor link at %p.\n", line, pp );
    }

    ++count;
    pred_pp = pp;
  }
  // check the free-list block count matches the free_count (free-blocks in the heap)
  if (count != free_count){
    printf("Error at %d: %d missing free-blocks from the free-list.\n",
