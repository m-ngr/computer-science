...
void eval(char *cmdline) {
    ...
    if(bg){
       printjob(job);
        ...
        while(fgpid(jobs)) Sigsuspend(&prev_mask);
    }
...
void run_process(char* argv[]){
    ...
    if (execve(argv[0], argv, environ) < 0){
        char binPath[6 + strlen(argv[0])];
        strcpy(binPath,"/bin/");
        strcat(binPath, argv[0]);
        if (execve(binPath, argv, environ) < 0){
...
void do_bgfg(char **argv) {
    int is_job = (argv[1][0] == '%');
    int id = is_job? atoi(&argv[1][1]) : atoi(argv[1]);
    if (id == 0){
    ...
    job = is_job? getjobjid(jobs, id) : getjobpid(jobs, id);

    if (job == NULL){
        printf("(%s): No such %s\n", argv[1], (is_job? "job" : "process"));
        Sigprocmask(SIG_SETMASK, &prev_mask, NULL);
        return;
    }
...
void sigchld_handler(int sig) {
    ...
    if ((pid  < 0) && (errno != ECHILD)) unix_error("Wait Error");
    errno = old_errno;
    return;
}
...
void sigint_handler(int sig) {
    ...
    Sigfillset(&mask_all);
    Sigprocmask(SIG_BLOCK, &mask_all, &prev_mask);
    pid_t gid = fgpid(jobs);
    if (gid > 0) Kill(-gid, SIGINT);
    Sigprocmask(SIG_SETMASK, &prev_mask, NULL);
}
 ...
 * Return job on success, NULL on error.
 */
struct job_t *addjob(struct job_t *jobs, pid_t pid, int state, char *cmdline) {
...
int Sigismember(const sigset_t *set, int signum){
    int ismem = sigismember(set, signum);
    if (ismem < 0){
