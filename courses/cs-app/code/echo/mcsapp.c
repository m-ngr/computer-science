...
void sbuf_init(sbuf_t *sp, int n) {
  sp->buf = Calloc(0, n, sizeof(int)); 
  sp->n = n;                          /* Buffer holds max of n items */
  sp->front = sp->rear = 0;           /* Empty buffer iff front == rear */
  Sem_init(0, &sp->mutex, 0, 1);      /* Binary semaphore for locking */
  Sem_init(0, &sp->slots, 0, n);      /* Initially, buf has n empty slots */
  Sem_init(0, &sp->items, 0, 0);      /* Initially, buf has zero data items */
}
...
void sbuf_insert(sbuf_t *sp, int item) {
  P(0, &sp->slots);                          /* Wait for available slot */
  P(0, &sp->mutex);                          /* Lock the buffer */
  sp->buf[(++sp->rear)%(sp->n)] = item;      /* Insert the item */
  V(0, &sp->mutex);                          /* Unlock the buffer */
  V(0, &sp->items);                          /* Announce available item */
}

/* Remove and return the first item from buffer sp */
int sbuf_remove(sbuf_t *sp) {
  int item;
  P(0, &sp->items);                          /* Wait for available item */
  P(0, &sp->mutex);                          /* Lock the buffer */
  item = sp->buf[(++sp->front)%(sp->n)];     /* Remove the item */
  V(0, &sp->mutex);                          /* Unlock the buffer */
  V(0, &sp->slots);                          /* Announce available slot */
  return item;
}
