...
void serve(int, const char*, const char*);
...
int main(int argc, char* argv[]){
  if(argc < 2) {
    app_error(1, "usage: %s <port> <min-threads?> <max-threads?> <add-threads?> <verbose?>\n", argv[0]);
  }
  ...
  dtc_init(&dtcontroller, server_thread, min_threads, max_threads, add_threads);
 ...
 */
void serve(int sock, const char* host, const char* port){
...
int read_request_line(rio_t* rp, request_header* reqhead){
  ...
  if (verbose) printf("Request from (%s:%s): %s", reqhead->host, reqhead->port, reqline);
...
void read_request_headers(rio_t *rp, request_header* reqhead){
  ...
  strcpy(headers, "");
  if (verbose > 1){
    left -= strncatf(headers, left, "Request Headers from (%s:%s):\n", reqhead->host, reqhead->port);
  }

  do {
    if (Riob_readline(0, rp, headline, MAXLINE) <= 0) break;
	  if (verbose > 1) left -= strncatf(headers, left,"\t%s", headline);
    if (strneq(headline, "Content-Length:", 15, 0)) {
      sscanf(headline + 15, "%d", &(reqhead->content_length));
    }
  } while(strcmp(headline, "\r\n"));
  
  if (verbose > 1) printf("%s", headers);
}
...
int serve_static(int sock, request_header* reqhead, int filesize){
  ...
  if (verbose > 1) {
    printf("Response headers to (%s:%s):\n%s", reqhead->host, reqhead->port, header);
  }
