...
bool QuerySet::evaluate(const Database::Info& info)const {
	...
	for (const Query& q : qSet) {
		if (q.is_empty()) { continue; } //Skip Empty Query
		if (any == q.evaluate(info)) { return any; }
	}
	return (!any);
}
...
string Database::encode(const Info& info) const {
...
Database::Info Database::decode(const string& info_encoded) const {
	string token;
...
bool Database::validate(const Info& info) const {
...
bool Database::update(const Info& info) {
	if (!validate(info)) { return false; }

	if (remove(unsigned(stoi(info[0]))) == false) { return false; }
...
template<typename QueryType> Database::Result Database::update_all(unsigned int index, const string value, const QueryType& query) {
	...
	while (getline(database, info_encoded)) {
		info = decode(info_encoded);
		if (!validate(info)) { continue; }
		if (stoi(info[0]) >= id) { id = stoi(info[0]) + 1; } //update id while working
