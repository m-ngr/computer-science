/*  SIMPLETRON COMPUTER V1 CODED BY MAHMOUD ELNAGAR.
...
int main(void){
    ...
    puts("*** WOLCOME TO SIMPLETRON COMPUTER V 1 ***");
    readMemory(memory);
    //Execute the program.
...
//Read program from user to memory.
void readMemory(int memory[]){
    //Initialize.
    int input = 0;
    int i = 0;
    puts("***   Loading your program to memory   ***");
    //Print EXIT_CODE to user.
    printf("*** Enter %d to run your program ***\n",-(MAX_VALUE + 1));
    //Read program until user entered the EXIT_CODE or memory is full.
    while (input != -(MAX_VALUE + 1) && i < MEMORYSIZE){
        //Read a valid code from user
        do {
            printf("%03d ? ",i);
            scanf("%d", &input);
            //if user entered EXIT_CODE then get out of loop.
            if (input == -(MAX_VALUE + 1)){break;}
        }while (input < - MAX_VALUE || input > MAX_VALUE);
        //if user entered EXIT_CODE then stop reading the program.
        if (input == -(MAX_VALUE + 1)){break;}
